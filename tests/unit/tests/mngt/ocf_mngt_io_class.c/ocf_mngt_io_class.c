 ...
 * <tested_function>ocf_mngt_io_classes_configure</tested_function>
 * <functions_to_leave>
...
static void ocf_mngt_io_classes_configure_test03(void **state)
{
	...
	struct ocf_mngt_io_class_config cfg[OCF_IO_CLASS_MAX] = {0};
	int result, i;
	...
	setup_valid_config(cfg, true);

	print_test_description("Remove all pratitions");
	...
	result = ocf_mngt_io_classes_configure(&cache, &cfg);
...
static void ocf_mngt_io_classes_configure_test01(void **state)
{
	...
	struct ocf_mngt_io_class_config cfg[OCF_IO_CLASS_MAX];
