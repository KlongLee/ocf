...
from pyocf.helpers import get_metadata_segment_size, get_metadata_segment_elems_count, get_metadata_segment_elems_per_page, get_metadata_segment_elem_size, get_metadata_segment_is_flapped, get_metadata_segment_page_location
...
@pytest.mark.parametrize("recovery,target_segment,expectation", [
    ...
    #(True, CacheMetadataSegment.COLLISION, does_not_raise()),
...
def test_metadata_corruption(
    ...
    corrupted_byte = get_random_target_in_segment(cache, target_segment)

    core = Core(core_volume)
    cache.add_core(core)

    core_exp_volume = CoreVolume(core, open=True)
    ...
    corrupt_byte(cache_volume.data, corrupted_byte)
...
@pytest.mark.parametrize("recovery,target_segment,expectation", [
    ...
    # Don't check collision - it may fail, or not fail
...
def test_metadata_corruption_standby_load(
    ...
    corrupted_byte = get_random_target_in_segment(secondary_cache, target_segment)

    secondary_cache_exp_obj = CacheVolume(secondary_cache, open=True)
    primary_cache_replicated_volume = ReplicatedVolume(primary_cache_volume, secondary_cache_exp_obj)
    ...
    corrupt_byte(secondary_cache_volume.data, corrupted_byte)

    with expectation:
        secondary_cache = Cache.load_standby_from_device(secondary_cache_volume, secondary_ctx, cache_line_size=cache_line_size)

    expectation.excinfo = None
...
def get_random_target_in_segment(cache: Cache, target_segment: CacheMetadataSegment):
    ...
    if target_page != page_count - 1:
        ...
        page_filled = elem_size
    else:
        page_filled = (elems_count % elems_per_page) * elem_size

    if target_page == CacheMetadataSegment.SB_CONFIG:
        # TODO: come up with something not-hardcoded for SB_CONFIG
        page_filled = 2148
