...
import pytest
import logging
import tests.security.conftest as conftest
...
def test_fuzzy_start_cache_mode(pyocf_ctx, cls, c_uint32_randomize):
    ...
    """
    if c_uint32_randomize not in CacheMode:
...
def test_fuzzy_start_cache_line_size(pyocf_ctx, c_uint64_randomize, cm):
    ...
    """
    if c_uint64_randomize not in CacheLineSize:
...
@pytest.mark.parametrize("cm", CacheMode)
@pytest.mark.parametrize("cls", CacheLineSize)
def test_fuzzy_start_id(pyocf_ctx, c_int_randomize, cm, cls):
    """
    Test whether it is impossible to start cache with invalid cache id value.
    :param pyocf_ctx: basic pyocf context fixture
    :param c_int_randomize: cache id value to start cache with
    :param cm: cache mode value to start cache with
    :param cls: cache line size value to start cache with
    """
    max_id_val = 1 << 14
    if c_int_randomize < 0 or c_int_randomize > max_id_val:
        with pytest.raises(OcfError, match="OCF_ERR_INVAL"):
            try_start_cache(cache_id=c_int_randomize, cache_mode=cm, cache_line_size=cls)
    else:
        logger.warning(f"Test skipped for valid cache id value: '{c_int_randomize}'. ")
...
def test_fuzzy_start_eviction_policy(pyocf_ctx, c_uint32_randomize, cm, cls):
    ...
    :param cls: cache line size value to start cache with
    """
    if c_uint32_randomize not in EvictionPolicy:
        with pytest.raises(OcfError, match="OCF_ERR_INVAL"):
...
def test_fuzzy_start_metadata_layout(pyocf_ctx, c_uint32_randomize, cm, cls):
    ...
    :param cls: cache line size value to start cache with
    """
    if c_uint32_randomize not in MetadataLayout:
        with pytest.raises(OcfError, match="OCF_ERR_INVAL"):
...
@pytest.mark.security
@pytest.mark.parametrize("cls", CacheLineSize)
@pytest.mark.parametrize('max_wb_queue_size', conftest.generate_random_numbers(c_uint32))
