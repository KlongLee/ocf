...
def test_start_params(pyocf_ctx, mode: CacheMode, cls: CacheLineSize, layout: MetadataLayout):
    ...
    logger.info("[ASSERT] Cache mode")
    assert stats["conf"]["cache_mode"] == mode
    logger.info("[ASSERT] Cache line size")
    assert stats["conf"]["cache_line_size"] == cls
    logger.info("[ASSERT] Eviction policy")
    assert stats["conf"]["eviction_policy"] == EvictionPolicy.DEFAULT
    logger.info("[ASSERT] Cache id")
    assert stats["conf"]["cache_id"] == cache_id
    logger.info("[ASSERT] Cache name")
    assert cache.get_name() == name
...
def test_stop(pyocf_ctx, mode: CacheMode, cls: CacheLineSize, with_flush: bool):
    ...
    logger.info("[ASSERT] Dirty data before MD5")
    stats = cache.get_stats()
    assert int(stats["conf"]["dirty"]) == (1 if mode == CacheMode.WB else 0)
    ...
    if mode == CacheMode.WB and not with_flush:
        ...
        logger.info("[ASSERT] MD5 check: core device vs exported object with dirty data")
        assert core_device.md5() != md5_exported_core
    else:
        logger.info("[ASSERT] MD5 check: core device vs exported object with clean data")
        assert core_device.md5() == md5_exported_core
...
def test_start_stop_multiple(pyocf_ctx):
    ...
    for i in range(1, caches_no):
        ...
        logger.info("[ASSERT] Cache line size")
        assert stats["conf"]["cache_line_size"] == cache_line_size
        logger.info("[ASSERT] Cache id")
        assert stats["conf"]["cache_id"] == i

    caches.sort(key=lambda e: randrange(1000))
    for cache in caches:
        logger.info("[ASSERT] Getting stats before stopping cache")
        stats = cache.get_stats()
        cache_id = stats["conf"]["cache_id"]
        cache.stop()
        logger.info("[ASSERT] Try getting cache after stopping it")
        assert get_cache_by_id(pyocf_ctx, cache_id) != 0
...
def test_100_start_stop(pyocf_ctx):
    ...
    for i in range(1, 101):
        ...
        assert stats["conf"]["cache_line_size"] == cache_line_size
        logger.info("[ASSERT] Cache id")
        assert stats["conf"]["cache_id"] == 1
        cache.stop()
        logger.info("[ASSERT] Try getting cache after stopping it")
        assert get_cache_by_id(pyocf_ctx, 1) != 0
    ...
    while run:
        if add:
            for i in range(0, randrange(3, 5) if increase else randrange(1, 3)):
                ...
                assert stats["conf"]["cache_line_size"] == cache_line_size
                logger.info("[ASSERT] Cache id")
                assert stats["conf"]["cache_id"] == len(caches)
...
def run_io_and_cache_data_if_possible(exported_obj, mode, cls):
    ...
    logger.info("[ASSERT] Occupancy")
    assert stats["usage"]["occupancy"]["value"] == \
        ((cls / CacheLineSize.LINE_4KiB) if mode != CacheMode.PT else 0)
...
def io_to_core(exported_obj: Core, data: Data, offset: int, to_core_device=False):
    ...
    logger.info("[ASSERT] IO to exported object completion")
...
def io_from_exported_object(exported_obj: Core, buffer_size: int, offset: int):
    ...
    logger.info("[ASSERT] IO from exported object completion")
    assert completion.results["err"] == 0
    return read_buffer
...
def check_stats_read_empty(exported_obj: Core, mode: CacheMode, cls: CacheLineSize):
    ...
    logger.info("[ASSERT] Cache mode")
    assert stats["conf"]["cache_mode"] == mode
    logger.info("[ASSERT] Writes to cache device")
    assert exported_obj.cache.device.get_stats()[IoDir.WRITE] == (0 if mode == CacheMode.PT else 1)
    logger.info("[ASSERT] Reads from core device")
    assert exported_obj.device.get_stats()[IoDir.READ] == 1
    logger.info("[ASSERT] Read full misses")
...
def check_stats_read_after_write(exported_obj, mode, cls, write_to_empty=False):
    ...
    logger.info("[ASSERT] Reads from cache device")
    ...
    logger.info("[ASSERT] Read hits")
    assert stats["req"]["rd_hits"]["value"] == \
        (1 if mode in {CacheMode.WT, CacheMode.WB} or (mode == CacheMode.WA and not write_to_empty) else 0)
    logger.info("[ASSERT] Occupancy")
    assert stats["usage"]["occupancy"]["value"] == (0 if mode == CacheMode.PT else (cls / CacheLineSize.LINE_4KiB))


def check_md5_sums(exported_obj: Core, mode: CacheMode):
    logger.info("[ASSERT] MD5 check: core device vs exported object")
    if mode == CacheMode.WB:
        logger.info("[ASSERT] MD5 check: core device vs exported object without flush")
        assert exported_obj.device.md5() != exported_obj.exp_obj_md5()
        exported_obj.cache.flush()
        logger.info("[ASSERT] MD5 check: core device vs exported object after flush")
        assert exported_obj.device.md5() == exported_obj.exp_obj_md5()
    else:
        logger.info("[ASSERT] MD5 check: core device vs exported object")
        assert exported_obj.device.md5() == exported_obj.exp_obj_md5()
