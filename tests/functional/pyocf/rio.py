...
class IoGen:
    def __init__(self, seed, extent, blocksize=512, random=False, randommap=True):
        self.seed = seed
        self.random = random
    ...
    def __next__(self):
        if self.random and not self.randommap:
            return choice(self.gen)
...
class Rio:
        ...
        def get_io_cb(self):
            ...
            def cb(error):
                if error != 0:
                    ...
                    if self.jobspec.continue_on_error == False:
        ...
        def run(self):
            ...
            while not self.should_finish():
                ...
                io = self.jobspec.target.new_io(
                    self.jobspec.target.cache.get_default_queue(),  # TODO what about multiple queues?
                    next(iogen),
                    self.jobspec.bs,
                    iodir,
                    0,
    ...
    def run_async(self):
        ...
        if not self.jobs:
            for _ in range(self.global_jobspec.njobs):
                jobs.append(self.global_jobspec)
