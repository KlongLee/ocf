...
{
	uint32_t ev_list = (cline % OCF_NUM_LRU_LISTS);
	ocf_part_id_t part_id;
	struct ocf_part *part;

	part_id = ocf_metadata_get_partition_id(cache, cline);

	ENV_BUG_ON(part_id == PARTITION_UNSPECIFIED);
		...
		struct ocf_part *src_part, uint32_t cline_no)
{
	...
	unsigned lru_idx;
	unsigned req_idx = 0;
	struct ocf_part *dst_part = &cache->parts[req->part_id];
...
void ocf_lru_populate(ocf_cache_t cache, ocf_cache_line_t num_free_clines)
{
	...
	for (i = 0; i < num_free_clines; i++) {
		...
		ev_list = (cline % OCF_NUM_LRU_LISTS);
