...
/**
 * @brief function loads individual properties from metadata
 * @param cache_volume volume from which to load metadata
 * @return 0 upon successful completion
 */
int ocf_metadata_load_properties(ocf_volume_t volume,
		ocf_metadata_load_properties_end_t cmpl, void *priv)
{
	OCF_DEBUG_TRACE(cache);

	return ocf_metadata_read_sb(volume->cache->owner, volume,
...
int ocf_metadata_probe(ocf_ctx_t ctx, ocf_volume_t volume,
		ocf_metadata_probe_end_t cmpl, void *priv)
{
	OCF_CHECK_NULL(ctx);
	OCF_CHECK_NULL(volume);

	return ocf_metadata_read_sb(ctx, volume, ocf_metadata_probe_cmpl,
			cmpl, priv);
}
