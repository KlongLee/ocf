...
static int get_data_to_flush(struct alru_flush_ctx *fctx)
{
	...
	while (fctx->part_id >= OCF_IO_CLASS_ID_MIN) {
		cache_line =
			parts[fctx->part_id]->runtime->cleaning.policy.alru.lru_tail;
...
static void alru_clean_complete(void *priv, int err)
{
	...
	interval = (fctx->part_id >= OCF_IO_CLASS_ID_MIN) ? 0 :
		config->thread_wakeup_time * 1000;

	fctx->cmpl(&fctx->cache->cleaner, interval);
}

static bool alru_clean(struct alru_flush_ctx *fctx)
{
	ocf_cache_t cache = fctx->cache;
	int to_clean;
