		...
		struct ocf_request *req, ocf_req_async_lock_cb cmpl)
{
	...
	lock = alock->cbs->lock_lines_fast(alock, req, OCF_READ);
	...
	if (lock != OCF_LOCK_ACQUIRED) {
		...
		status = alock->cbs->lock_lines_slow(alock, req, OCF_READ, cmpl);
		if (!status) {
			if (env_atomic_dec_return(&req->lock_remaining) == 0) {
		...
		struct ocf_request *req, ocf_req_async_lock_cb cmpl)
{
	...
	lock = alock->cbs->lock_lines_fast(alock, req, OCF_WRITE);
	...
	if (lock != OCF_LOCK_ACQUIRED) {
		...
		status = alock->cbs->lock_lines_slow(alock, req, OCF_WRITE, cmpl);
		if (!status) {
			if (env_atomic_dec_return(&req->lock_remaining) == 0) {
				lock = OCF_LOCK_ACQUIRED;
				env_atomic_dec(&alock->waiting);
