...
static int _ocf_mngt_io_class_add(ocf_cache_t cache,
		...
		const struct ocf_mngt_io_class_config *cfg)
{
	...
	if (cfg->name) {
		result = _ocf_mngt_io_class_add(cache, cfg);
	} else {
		...
		const struct ocf_mngt_io_class_config *cfg)
{
	...
	for (i = 0; i < OCF_IO_CLASS_MAX; i++) {
		result = _ocf_mngt_io_class_validate_cfg(cache, &cfg[i]);
		if (result)
			return result;
	}
	...
	for (i = 0; i < OCF_IO_CLASS_MAX; i++) {
		result = _ocf_mngt_io_class_configure(cache, &cfg[i]);
