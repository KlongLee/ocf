...
static void _ocf_mngt_end_flush(ocf_pipeline_t pipeline, void *priv,
		ocf_pipeline_arg_t arg)
{
	struct ocf_mngt_cache_flush_context *context = priv;
	ocf_cache_t cache = context->cache;
...
typedef void (* ocf_flush_containters_coplete_t)(void *context, int error);

void _ocf_flush_containter_coplete(struct flush_container *fc)
{
	struct ocf_mngt_cache_flush_context *context = fc->context;

	env_vfree(fc->flush_data);

	if (atomic_dec_return(&context->fcs.count)) {
		return;
	}

	env_vfree(context->fcs.fctbl);
		...
		uint32_t fcnum, ocf_flush_complete_t complete)
{
	...
	env_vfree(fctbl);
	complete(context, env_atomic_read(&context->fcs.error));
}
		...
		struct ocf_mngt_cache_flush_context *context, int error)
{
	...
	if (ocf_mngt_cache_is_dirty(cache)) {
		ocf_pipeline_repeat(context->pipeline);
		return;
	}
...
static struct ocf_pipeline_properties _ocf_mngt_cache_flush_pipeline_properties = {
	...
	.steps = {
		...
		OCF_PL_STEP(_ocf_mngt_end_flush),
		...
		struct ocf_mngt_cache_flush_context *context, int error)
{
	...
	/* TODO: shouldn't we finish in case of any error, like it's done in
	 * flush cache (ocf_mngt_flush_all_cores_complete) ? */
	if (error == -OCF_ERR_FLUSHING_INTERRUPTED ||
			error == -OCF_ERR_WRITE_CORE) {
		ocf_pipeline_finish(context->pipeline, error);
		return;
	}

	if (env_atomic_read(&cache->core_runtime_meta[core_id].dirty_clines))  {
		ocf_pipeline_repeat(context->pipeline);
		return;
	}
